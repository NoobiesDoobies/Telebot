...
def send_about(message):
    ...
    with open('template_text/about.txt', mode='r', encoding='utf-8') as f:
        ...
        STUDENT_NAME = "Carlios Eryan",
        BATCH_ACADEMY = "SPARTA",
        GITHUB_REPO_LINK = "https://github.com/NoobiesDoobies/Telebot"
        )
...
df["ad_id"] = df["ad_id"].astype("string").astype("object")
...
def send_plot(message):
    ...
    if selected_campaign_id in unique_campaign:
        ...
        df_campaign = df[df["campaign_id"] == selected_campaign_id]
        df_plot = df_plot = df_campaign.groupby("age").agg({
            "spent" : "sum",
        ...
        plot_info = list(zip(
        ["total spent", "approved conversion", "cost per conversion"],
        df_plot.idxmax(),
        df_plot.idxmin()))
...
def echo_all(message):
    ...
    with open('template_text/default.txt', mode='r', encoding='utf-8') as f:
        ...
        sweat_smile = emoji.emojize(":grinning_face_with_sweat:")
        default = temp.substitute(EMOJI = sweat_smile)
